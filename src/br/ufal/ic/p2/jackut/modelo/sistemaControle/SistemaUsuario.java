...
public class SistemaUsuario {
    ...
    /*public String getAtributoUsuario(String login, String atributo) throws UsuarioNaoCadastradoException, LoginInvalidoException, SenhaInvalidaException, AtributoNaoPreenchidoException {
    
        int id = getIdUsuario(login);
        String str = "";
        if(dados.usuariosPorID.containsKey(id)){
            ...
            switch (atributo){
                case "nome":
                    str = usuario.getNome();
                case "descricao":
                    str = usuario.getDescricao();
                case "estadoCivil":
                    str = usuario.getEstadoCivil();
                case "aniversario":
                    str = usuario.getAniversario();
                case "filhos":
                    str = usuario.getFilhos();
                case "idiomas":
                    str = usuario.getIdiomas();
                case "cidadeNatal":
                    str = usuario.getCidadeNatal();
                case "estilo":
                    str = usuario.getEstilo();
                case "fumo":
                    str = usuario.getFumo();
                case "bebo":
                    str = usuario.getBebo();
             ...
             System.out.println("ATRIBUTO  FORA DO   switch: "+str);
        }
        else{
...
if (dados.usuariosPorID.containsKey(ID)) {
        ...
        if(verificaAmizade(usuario.getSolicitacoesEnviadas(), amigo)){
           ...
           if(verificaAmizade(usuario1.getSolicitacoesEnviadas(), usuario.getLogin())){
            System.out.println(amigo+" TAMBEM ME MANDOU SOLICITACAO");
                ...
                System.out.println("MINHA LISTA DE AMIGOS: "+usuario.getAmigos());
                System.out.println("A LISTA Do AMIGOS: "+usuario1.getAmigos());
                //atualizar solicitacoes enviadas

                System.out.println("MINHA LISTA DE SOLICITACOA ANTES DE ATT: "+usuario.getSolicitacoesEnviadas());
                System.out.println("LISTA DE SOLICITACOA do amigo ANTES DE ATT: "+usuario.getSolicitacoesEnviadas());
                ...
                System.out.println("MINHA LISTA DE SOLICITACOA DEPOIS DE ATT: "+usuario.getSolicitacoesEnviadas());
                System.out.println("LISTA DE SOLICITACOA do amigo DEPOIS DE ATT: "+usuario.getSolicitacoesEnviadas());
    ...
    private boolean verificaAmizade(String listaAmigos, String amigo) throws LoginInvalidoException, AtributoNaoPreenchidoException{
        ...
        if(listaAmigos.contains(",")){
            ...
            System.out.println("A LISTA apos split ASSIM: "+listaAmigosArray);
